...
export default function NavBar(){
    ...
    return(
        ...
        <nav className="w-full relative flex justify-between items-center px-8 py-4 border-b border-orange-200 ">
            ...
            <button className="lg:hidden flex md:flex cursor-pointer mr-5 text-orange-500" onClick={() => setIsOpen(!isOpen)}>{isOpen ? <FaTimes size={25}/> : <FaBars size={25} />}</button>
            {isOpen && (
                <motion.div {...scrollUp} className="lg:hidden w-full h-auto flex flex-col absolute top-17 z-1000 left-0 bg-gradient-to-r from-orange-500 to-orange-400 p-2">
