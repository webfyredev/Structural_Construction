    ...
    return(
        <>
            ...
            <div className="w-full h-auto grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 lg:px-10 p-5 gap-5">
                {services.map((data) =>(
                    <motion.div key={data.id} {...scrollRight} className="w-full h-75 md:h-85 mb-5 relative group rounded-xl overflow-hidden">
                        <img src={data.img} className="w-full h-full object-cover transition-all duration-500 rounded-xl hover:scale-110"/>
                        <div className="absolute inset-0 bg-black/60 w-full h-[18%] mt-62 md:mt-70 rounded-b-xl">
                            <div className="relative w-full h-full">
                                <div className="absolute w-12 h-12 top-[-65%] right-5 rounded-full hover:cursor-pointer bg-gradient-to-r from-orange-500 to-orange-400 flex items-center justify-center text-white text-xl animate-bounce transition-all duration-300">
                                    {data.icon}
                                </div>
                                <a href="#" className="absolute text-white font-semibold m-3 md:m-5 text-md hover:text-orange-100 transition">{data.service}</a>
                            </div>
                        </div>
                    </motion.div>
                ))}
